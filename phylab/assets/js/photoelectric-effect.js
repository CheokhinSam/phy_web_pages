...
function initPhotoelectricEffect() {
    ...
    function freqToColor(f) {
        // 低頻 = 紅外/紅（長波長），高頻 = 紫外/紫（短波長）
        if (f < 380) return { r: 120, g: 30, b: 160 };
        if (f < 480) return { r: 140, g: 40, b: 200 };
        if (f < 510) return { r: 70, g: 50, b: 180 };
        if (f < 540) return { r: 40, g: 120, b: 180 };
        if (f < 580) return { r: 60, g: 170, b: 50 };
        if (f < 620) return { r: 220, g: 190, b: 30 };
        if (f < 680) return { r: 220, g: 120, b: 20 };
        if (f < 750) return { r: 200, g: 40, b: 40 };
        return { r: 60, g: 0, b: 0 };
    }
