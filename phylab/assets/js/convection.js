...
function initConvection() {
    ...
    function updateCards() {
        ...
        // 流體性質：ν（運動粘度）、α（熱擴散係數）、β（熱膨脹係數）
        const fluidProps = {
            0.08: { nu: 1.0e-6, alpha: 1.4e-7, beta: 2.1e-4 },  // 水
            0.25: { nu: 5.0e-5, alpha: 8.0e-8, beta: 7.0e-4 },   // 油
            0.6:  { nu: 5.0e-4, alpha: 1.3e-7, beta: 3.0e-4 }    // 蜂蜜
        };
        const fp = fluidProps[viscosity] || fluidProps[0.08];
        let Ra = Math.round(9.8 * fp.beta * dT * Math.pow(0.1, 3) / (fp.nu * fp.alpha));
