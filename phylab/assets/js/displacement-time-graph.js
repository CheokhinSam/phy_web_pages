/**
 * 📈 位移時間圖 — 觀察不同運動的位移-時間圖形特徵
 * 等速、等加速度、減速、靜止四種模式，即時繪製 x-t 圖。
 */
function initDisplacementTimeGraph() {
    ...
    if (ctrlPanel) {
        ctrlPanel.innerHTML = `
            ...
            ${PhysicsUtils.formulaBox('核心公式', 'x = v_0 t + \\frac{1}{2}at^2')}
            <div class="control-box" style="margin-top: 12px;">
            ...
            <div id="refPanel" style="display: none; margin-top: 12px; padding: 12px; background: #fff; border: 2px solid #000; border-radius: 6px;">
                <div style="font-weight: 900; font-size: 0.85rem; margin-bottom: 10px; letter-spacing: 0.5px;">常見 x-t 圖形</div>
        ...
        const refData = [
            ...
            { name: '等速（正向）', desc: 'v > 0', fn: t => t * 2, color: '#22c55e' },
            { name: '等速（反向）', desc: 'v < 0', fn: t => -t * 2, color: '#8b5cf6' },
            { name: '等加速度', desc: 'a > 0', fn: t => 0.5 * 3 * t * t, color: '#2563eb' },
            { name: '減速停止', desc: 'a < 0', fn: t => { const ts = 3; return t < ts ? 6 * t - 0.5 * 3 * t * t : 6 * ts - 0.5 * 3 * ts * ts; }, color: '#ef4444' },
            { name: '先正後負', desc: 'v 先+後−', fn: t => { const ts = 2; return t < ts ? 3 * t : 3 * ts - 3 * (t - ts); }, color: '#f59e0b' },
        ];
        ...
        refData.forEach(item => {
            ...
            cvs.addEventListener('click', () => {
                ...
                else if (item.name === '等加速度') { modeSelect.value = 'accelerate'; v0Slider.value = 0; aSlider.value = 3; v0 = 0; a = 3; document.getElementById('v0Val').textContent = '0.0'; document.getElementById('aVal').textContent = '3.0'; }
                else if (item.name === '減速停止') { modeSelect.value = 'decelerate'; v0Slider.value = 6; aSlider.value = 3; v0 = 6; a = 3; document.getElementById('v0Val').textContent = '6.0'; document.getElementById('aVal').textContent = '3.0'; }
                else if (item.name === '先正後負') { modeSelect.value = 'accelerate'; v0Slider.value = 3; aSlider.value = -3; v0 = 3; a = -3; document.getElementById('v0Val').textContent = '3.0'; document.getElementById('aVal').textContent = '-3.0'; }
    ...
    const updateCards = PhysicsUtils.createDataCards([
        ...
        { label: '位移 POS', id: 'cardX', unit: 'm', highlight: true },
    ...
    function drawSim() {
        ...
        const { x, v, acc } = calcPhysics(simTime);
        ...
        ctx.textAlign = 'left';
        ctx.fillText('x-t 圖  DISPLACEMENT-TIME', graphLeft, graphTop - 4);
        ...
        for (let i = -ySteps; i <= ySteps; i++) {
            const val = (maxX / ySteps) * i;
            const py = graphTop + graphH / 2 - (val / maxX) * (graphH / 2);
            if (py >= graphTop && py <= graphTop + graphH) {
                ctx.fillText(val.toFixed(0) + 'm', graphLeft - 4, py + 3);
            }
        }
        ...
        if (trail.length > 1) {
            ...
            for (const pt of trail) {
                ...
                const py = zeroY - (pt.x / maxX) * (graphH / 2);
            ...
            const lpy = zeroY - (lastPt.x / maxX) * (graphH / 2);
    ...
    PhysicsUtils.setupResetBtn('resetBtn', () => {
        ...
        updateCards({ cardTime: '0.00', cardX: '0.00', cardV: '0.00', cardA: '0.0' });
    });
