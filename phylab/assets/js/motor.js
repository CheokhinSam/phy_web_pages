...
function initMotor() {
    ...
    if (ctrlPanel) {
        ctrlPanel.innerHTML = `
            ...
            <div class="control-box" style="margin-top: auto; border-top: 2px solid #000; padding-top: 12px; background: #fff;">
                <label style="margin-bottom: 4px; color: #64748b; font-size: 0.75rem; font-weight: 700; letter-spacing: 0.5px;">BACK-EMF / 反電動勢</label>
                <div style="font-size: 0.95rem; font-weight: 900; font-family: Cambria, serif; color: #000; margin-bottom: 4px;">$$\\varepsilon = N \\cdot B \\cdot A \\cdot \\omega$$</div>
                <div style="font-size: 0.95rem; font-weight: 900; font-family: Cambria, serif; color: #000; margin-bottom: 4px;">$$I = \\frac{V - \\varepsilon}{R}$$</div>
    ...
    function updateCards() {
        ...
        let torque = numTurns * magneticField * coilArea * current;
    ...
    // ============================================================
    // D. 物理引擎（含反電動勢 back-EMF）
    // ============================================================
    const coilArea = 0.005;   // 線圈面積 A (m²)，歸一化常數
    const momentOfInertia = 0.05; // 轉動慣量 I (kg·m²)
    ...
    function update(dt) {
        ...
        // 反電動勢 ε = N·B·A·ω
        const backEMF = numTurns * magneticField * coilArea * angularVel;
        // 電流 I = (V - ε) / R，電動勢不能為負（整流器效果）
        current = Math.max(0, (voltage - backEMF) / resistance);
        ...
        // 角加速度 α = τ/I - 摩擦
        angularVel += (effectiveTorque / momentOfInertia) * dt;
        angularVel *= (1 - 0.8 * dt);
    ...
    function draw() {
        ...
        if (isOn && current > 0.01) {
            let forceMag = numTurns * magneticField * coilArea * current;
