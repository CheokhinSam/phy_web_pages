...
function initFreeFall() {
    ...
    if (ctrlPanel) {
        ctrlPanel.innerHTML = `
            ...
            <div class="control-box">
                <label>
                    <span>阻力係數 <i>k</i></span>
                    <span style="font-family: monospace; font-weight: 800;">
                        <span id="airVal" style="color: #2563eb;">0.0</span> kg/m
                    </span>
                </label>
    ...
    const updateCards = PhysicsUtils.createDataCards([
        ...
        { label: '🍎 終端速度', id: 'cardVt1', unit: 'm/s' },
        { label: '⚙️ 終端速度', id: 'cardVt2', unit: 'm/s' },
    ]);

    // ==========================================================================
    // E. 物理計算（速度相關阻力 F_d = kv²）
    // ==========================================================================
    function calcPhysics(t, mass) {
        ...
        // k = airResistance（滑桿值直接作為阻力係數）
        const k = airResistance;
        // 終端速度 v_t = sqrt(mg/k)
        const vTerm = Math.sqrt(mass * g / k);
        // 解析解：v(t) = v_t * tanh(gt/v_t)
        const v = vTerm * Math.tanh(g * t / vTerm);
        // 落下距離：y(t) = (v_t²/g) * ln(cosh(gt/v_t))
        const fallen = (vTerm * vTerm / g) * Math.log(Math.cosh(g * t / vTerm));
        // 瞬時加速度：a = g - (k/m)v²
        const a = g - (k / mass) * v * v;
        return { fallen, v, a: Math.max(a, 0) };
    }
    ...
    function resetSimulation() {
        ...
        updateCards({ cardTime: '0.00', cardV1: '0.00', cardV2: '0.00', cardVt1: '-', cardVt2: '-' });
    }
    ...
    function loop(ts) {
        ...
        const vt1 = airResistance > 0 ? Math.sqrt(obj1.mass * g / airResistance).toFixed(1) : '∞';
        const vt2 = airResistance > 0 ? Math.sqrt(obj2.mass * g / airResistance).toFixed(1) : '∞';
        updateCards({
            cardTime: simTime.toFixed(2),
            cardV1: phys1.v.toFixed(2),
            cardV2: showSecond ? phys2.v.toFixed(2) : '-',
            cardVt1: vt1,
            cardVt2: showSecond ? vt2 : '-'
        });
